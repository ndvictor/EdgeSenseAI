...
STATUS_LABELS: dict[str, str] = {
    "created": "Created",
    "created_partial": "Created (partial)",
    "partial": "Partial",
...
def _is_created_bucket(status: StatusValue) -> bool:
    return status == "created"


def _is_partial_bucket(status: StatusValue) -> bool:
    return status in ("partial", "created_partial")
...
def _apply_inventory_overrides(units: list[dict[str, Any]]) -> None:
    ...
    for unit_name, (ui_route, ep) in completed_v1.items():
        apply(
            ...
            frontend="created",
            test="tested",
            implementation_status="created_partial",
            route=ui_route,
            endpoint_family=ep,
            notes=["Reconciled: backend+frontend+contract tests in place (v1)."],
        )
...
def _summarize_units(units: list[dict[str, Any]]) -> dict[str, Any]:
    created = sum(1 for u in units if _is_created_bucket(u["status"]))
    ...
    backend_created_count = sum(1 for u in units if u.get("backend_status") == "created")
    frontend_created_count = sum(1 for u in units if u.get("frontend_status") == "created")
    tested_count = sum(1 for u in units if u.get("test_status") == "tested")
    missing_count = sum(1 for u in units if u.get("backend_status") != "created" and _is_missing_bucket(u["status"]))
    needs_backend_count = sum(1 for u in units if u.get("backend_status") != "created")
    needs_frontend_count = sum(1 for u in units if u.get("backend_status") == "created" and u.get("frontend_status") != "created")
...
def _stage_summary_for_units(stage_units: list[dict[str, Any]]) -> dict[str, int]:
    return {
        ...
        "created": sum(1 for u in stage_units if _is_created_bucket(u["status"])),
...
def _component_categories(units: list[dict[str, Any]]) -> list[dict[str, Any]]:
    buckets: dict[str, dict[str, int]] = {c: {"total": 0, "created": 0, "partial": 0, "missing": 0, "backlog": 0} for c in COMPONENT_CATEGORY_ORDER}
    for u in units:
