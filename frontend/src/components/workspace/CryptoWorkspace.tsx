  ...
  return (
    <div className="space-y-4">
      ...
      {!snapshot || !pipeline || !feasibility || !risk ? (
        <div className="py-8 text-center text-sm text-slate-400">Loading crypto workflow...</div>
      ) : (
        <div className="space-y-4">
          <section className={wsSection}>
            <h2 className="mb-3 text-lg font-semibold text-emerald-300">BTC readiness snapshot</h2>
          ...
          <section className="grid grid-cols-1 gap-4 xl:grid-cols-3">
            <div className={wsSection}>
              <h3 className="text-lg font-semibold text-emerald-300">Crypto model pipeline</h3>
              <div className="mt-3 space-y-3 text-sm text-slate-300">
                <p>
                  Directional bias: <span className="font-bold text-white">{pipeline.directional_bias}</span>
                </p>
                <p>
                  Regime bias: <span className="font-bold text-white">{pipeline.regime_bias}</span>
                </p>
                <p>
                  Volatility fit: <span className="font-bold text-white">{pipeline.volatility_fit}</span>
                </p>
                <p>
                  Ranker score: <span className="font-bold text-emerald-300">{pipeline.ranker_score}</span>
                </p>
              </div>
            </div>
            <div className={wsSection}>
              <h3 className="text-lg font-semibold text-emerald-300">Account routing</h3>
            ...
            <div className={wsSection}>
              <h3 className="text-lg font-semibold text-emerald-300">Risk check</h3>
              <p className="mt-3 text-sm text-slate-300">
                Reward/Risk: <span className="font-bold text-white">{risk.reward_risk_ratio.toFixed(1)}R</span>
              </p>
              <p className="mt-2 text-sm text-slate-300">
                Stop distance: <span className="font-bold text-white">{risk.stop_distance_percent.toFixed(2)}%</span>
              </p>
              <p className="mt-2 text-sm text-slate-300">
                Status: <span className={risk.passed ? "font-bold text-emerald-300" : "font-bold text-amber-300"}>{risk.risk_status}</span>
              </p>
            </div>
          </section>

          <section className={wsSection}>
            <h2 className="mb-3 text-lg font-semibold text-emerald-300">Crypto strategy lanes</h2>
