  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-bold ${
        ready ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300" : "border-rose-500/40 bg-rose-500/10 text-rose-300"
      }`}
    >
    ...
    <article className="overflow-hidden rounded-2xl border border-emerald-400/15 bg-black/35 shadow-[0_0_40px_rgba(0,0,0,0.25)] backdrop-blur">
      ...
      <div className="grid grid-cols-1 divide-y divide-emerald-400/10 border-t border-emerald-400/10 md:grid-cols-3 md:divide-x md:divide-y-0">
        ...
        <div className="p-4">
          ...
          <p className="mt-2 text-sm font-bold text-slate-200">{strategy.retestFrequency}</p>
        </div>
        <div className="p-4">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Next retest</p>
          <p className="mt-2 text-sm font-bold text-slate-200">{strategy.nextRetest}</p>
        </div>
      </div>
    </article>
  );
...
export function StrategiesPanel({ showHeader = false }: { showHeader?: boolean }) {
  const tabs = [
    ["idea", "Idea"],
    ["research", "Research"],
  ...
  const allStrategies = useMemo(() => [...readyStrategies, ...notReadyStrategies], []);

  const visible = useMemo(() => allStrategies.filter((s) => s.stage === activeTab), [allStrategies, activeTab]);
      ...
      <section className="rounded-2xl border border-emerald-400/15 bg-black/35 p-4 shadow-[0_0_40px_rgba(0,0,0,0.25)] backdrop-blur">
        ...
        <div className="mt-4 space-y-4">
          {visible.length > 0 ? (
            ...
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-200">No strategies assigned to this stage yet.</div>
          )}
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-300">
            Ready status does not enable live execution. All strategies remain paper/research-only until a separate broker, risk, and approval design is implemented.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
          <div className="flex items-center gap-3">
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-300">
            Not-ready strategies require model availability, paper-test evidence, blocker resolution, and owner approval before they can move into production readiness.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
          <div className="flex items-center gap-3">
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-300">
            Ready strategies show their latest validation report and retest cadence. Candidate strategies should enter the research/backtest queue before promotion.
          </p>
        </div>
      </section>
    </div>
